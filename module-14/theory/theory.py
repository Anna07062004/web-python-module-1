...
print(calculate_discount_bad("reqular", 10000))
...
def scan_document(device: Printer):
...
class BadOrderService:
    ...
    def complete(self, email: str, totle: float) -> None:
        print(f"Заказ на сумму {totle} оформлен")
...
class OrderService:
    def __init__(self, notiler: Notifiler):
        self.sender = notiler

    def complete_order(self, email: str, totle: float) -> None:
        print(f"Заказ на сумму {totle} оформлен")
        self.sender.send(email, "Ваш заказ оформлен")

OrderService(EmailNotifiler()).complete_order("user@example.com", 3500)
OrderService(SMSNotfiler()).complete_order("+789986777", 3500)
