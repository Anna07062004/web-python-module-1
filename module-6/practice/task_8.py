...
for messange in messages:
    user = messange["user"]
    messages_count[user] = messages_count.get(user, 0) + 1

    user = messange["user"]
    words = messange["text"].split()
...
for user_name in user_words:
    for messange in messages:
        if messange["user"] == user_name:
            for word in messange["text"].split():
...
commone_words = set.intersection(*user_words.values())
